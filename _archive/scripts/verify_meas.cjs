...
async function verify() {
  ...
  const [totalRows] = await connection.execute('SELECT COUNT(*) as total FROM questions WHERE topic_id = 4');
  console.log(`Total Measurement Questions (Topic 4): ${totalRows[0].total}`);

  const [diagRows] = await connection.execute(`
    SELECT grade, difficulty, COUNT(*) as count 
    FROM questions 
    WHERE topic_id = 4 AND distractor_diagnostics IS NOT NULL
  ...
  const [gradeTotals] = await connection.execute(`
    ...
    FROM questions 
    WHERE topic_id = 4 AND grade BETWEEN 1 AND 8
    GROUP BY grade 
    ORDER BY grade
  `);
