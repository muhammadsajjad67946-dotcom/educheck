...
export default function SummaryReport() {
  ...
  const [showDetailedSubtopics, setShowDetailedSubtopics] = useState(false)
  ...
  // Categorized 5 Core Strands with GLS scores and equal alignment formatting
  const coreStrandOrder = [
    { key: 'Geometry', label: 'Geometry', icon: '📐' },
    { key: 'Data Analysis', label: 'Data Analysis', icon: '📊' },
    { key: 'Number & Operations', label: 'Number & Operations', icon: '🔢' },
    { key: 'Algebra', label: 'Algebra', icon: '🔤' },
    { key: 'Measurement', label: 'Measurement', icon: '📏' },
  ]

  const categorizedStrands = useMemo(() => {
    const strandMap = {}
    coreStrandOrder.forEach((c) => {
      strandMap[c.key] = []
    })
  ...
  return (
    <div className={`mx-auto max-w-6xl space-y-8 p-4 sm:p-6 lg:p-8 transition-colors duration-300 ${darkMode ? 'text-slate-100' : 'text-slate-900'}`}>
      ...
      {/* Categorized 5 Core Strands Diagnostic Breakdown with Perfect Equal Alignment */}
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-xl font-bold flex items-center gap-2">
              <span>📊</span> Curriculum Strand Mastery & Diagnostic Subtests
            </h3>
            <p className="text-xs sm:text-sm text-slate-500">
              Evaluated across K–{targetGradeNum} standards categorized into 5 core mathematical domains
            </p>
          </div>

          <button
            type="button"
            onClick={() => setShowDetailedSubtopics(!showDetailedSubtopics)}
            className="inline-flex items-center gap-2 rounded-xl border border-sky-500/40 bg-sky-500/10 px-5 py-2.5 text-xs font-bold text-sky-500 hover:bg-sky-500/20 transition cursor-pointer self-start sm:self-auto"
          >
            {showDetailedSubtopics ? (
              <>
                <ChevronUp size={15} /> Hide Topics & Subtopics
              </>
            ) : (
              <>
                <ChevronDown size={15} /> 📋 View All Topics & Subtopics
              </>
            )}
          </button>
        </div>

        {/* 5 Core Strands Aligned Tables */}
        {showDetailedSubtopics && (
          <div className="mt-6 space-y-6">
            {coreStrandOrder.map((strand) => {
              const rows = categorizedStrands[strand.key] || []
              ...
              return (
                <div
                  ...
                  className={`rounded-2xl border overflow-hidden ${
                    darkMode ? 'border-white/10 bg-slate-900/40' : 'border-slate-200 bg-slate-50/60'
                  }`}
                >
                  {/* Strand Header */}
                  <div className={`px-5 py-3.5 flex items-center justify-between border-b ${
                    ...
                    <div className="flex items-center gap-2 font-bold text-sm sm:text-base">
                      <span className="text-lg">{strand.icon}</span>
                      <span className={darkMode ? 'text-white' : 'text-slate-900'}>{strand.label}</span>
                      <span className="text-xs font-medium text-slate-400">({masteredCount}/{totalCount} Mastered)</span>
                    </div>
